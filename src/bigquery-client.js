...
export class BigQueryClient {
  ...
  constructor(config) {
    ...
    logger.debug(`[BigQueryClient] Config - projectId: ${config.bigquery.projectId}, dataset: ${config.bigquery.dataset}, table: ${config.bigquery.table}, location: ${config.bigquery.location}`);
  ...
  async countPartition({ nodeId, clusterSize }) {
    logger.info(`[BigQueryClient.countPartition] Counting partition records - nodeId: ${nodeId}, clusterSize: ${clusterSize}`);
    ...
    try {
      ...
      logger.info(`[BigQueryClient.countPartition] Count complete - ${count} records in partition (took ${duration}ms)`);
      return count;
    } catch (error) {
