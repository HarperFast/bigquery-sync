...
describe('Column Selection - End-to-End Integration', { skip: !credentialsAvailable }, () => {
  ...
  before(async () => {
    ...
    bigqueryClient = new BigQueryClient({ bigquery: config });
      ...
      if (records && records.length > 0) {
        ...
        for (const field of recordFields) {
          assert.ok(
            ...
            `Field '${field}' should be in selected columns`
          );
        }
        ...
        for (const column of selectedColumns) {
          assert.ok(
            recordFields.includes(column),
            `Selected column '${column}' should be present in record`
          );
        }
  ...
  describe('Query Performance Comparison', () => {
    it('should demonstrate performance difference between wildcard and specific columns', async () => {
      ...
      if (wildcardRecords?.length > 0 && specificRecords?.length > 0) {
        ...
        assert.ok(
          specificFieldCount <= wildcardFieldCount,
          'Specific columns should return fewer or equal fields'
        );
